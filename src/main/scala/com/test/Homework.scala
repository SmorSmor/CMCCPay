    ...
    stream.foreachRDD({
      rdd =>
        ...
        /**
          ...
          * 求每月各手机号充值的平均金额
          */
        val baseRDD = rdd.map(_.value()).map(json => JSON.parseObject(json)).map(obj => {
          val month = obj.getString("date").substring(0, 7)
          val day = obj.getString("date").substring(0, 10)
          val phoneNum = obj.getString("phoneNum")
          val money = obj.getString("money")
          (month, day, phoneNum, money)
        }).cache()

        baseRDD.map(tup => (tup._2, tup._4))
          .foreachPartition(part => {
            val jedis = JedisConnectionPool.getConnection()
            part.foreach(tp => {
              jedis.select(1)
              jedis.hincrBy(tp._1, "total", tp._2.toLong)
            })
            jedis.close()
          })

        baseRDD.map(tup => (tup._1, tup._3, tup._4))
          .foreachPartition(part => {
            val jedis = JedisConnectionPool.getConnection()
            part.foreach(tp => {
              jedis.select(1)
              jedis.hincrBy("" + tp._1, "" + tp._2, tp._3.toLong)
            })
